...
def main(resol,name):
	...
	input_data=pd.read_csv("./cool2txt/Region3_vs_nontarget_Union_"+str(resol)+"kb"+name+"DE_results.csv",index_col=0)
	#### log2Folchange
	...
	sns.heatmap(tmp,vmax=2,vmin=0,square=True,cmap="Reds")
	for i, j in product(range(tmp.shape[0]), range(tmp.shape[1])):
		if tmp.index[i]+"."+tmp.columns[j] in input_data.index:
			if input_data.loc[tmp.index[i]+"."+tmp.columns[j],'padj']<=0.05 and input_data.loc[tmp.index[i]+"."+tmp.columns[j],'log2FoldChange']>0:
	...
	plt.tight_layout()
	plt.savefig("./figures/Region3_vs_nontarget_Union_"+str(resol)+"kb"+name+"DE_log2FC_heatmap_min_0.pdf")
	

main(10,"_")
#main(20,"_")
#main(10,"_include_target_region_")
#main(20,"_include_target_region_")
